    ...
    async for dialog in c.get_dialogs():
        if dialog.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
            ...
            try:
                ...
                error += 1
    return await load.edit(f"<i>Broadcast was sent to {done} groups, failed to send to {error} groups</i>")
    ...
    async for dialog in c.get_dialogs():
        if dialog.chat.type in enums.ChatType.PRIVATE:
            ...
            try:
                ...
                error += 1
    return await load.edit(f"<i>Broadcast was send to {done} users, failed to send to {error} users</i>")
    ...
    async for dialog in c.get_dialogs():
        if dialog.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
            ...
            try:
                ...
                error += 1
    return await load.edit(f"<i>Broadcast was send to {done} forward, failed to send to {error} forward")
